<?php

/**
 * WebHemi2
 *
 * LICENSE
 *
 * This source file is subject to the new BSD license that is bundled
 * with this package in the file LICENSE.txt.
 * It is also available through the world-wide-web at this URL:
 * http://webhemi.gixx-web.com/license/new-bsd
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to license@gixx-web.com so we can send you a copy immediately.
 *
 * @category     WebHemi2
 * @package      WebHemi2_Component
 * @subpackage   WebHemi2_Component_Image
 * @author       Gixx @ www.gixx-web.com
 * @copyright    Copyright (c) 2014, Gixx-web (http://www.gixx-web.com)
 * @license      http://webhemi.gixx-web.com/license/new-bsd   New BSD License
 */

namespace WebHemi2\Component\Image;

use Exception;
use WebHemi2\Component\Image\AbstractImage;

/**
 * WebHemi2 .PNG Image processing Component
 *
 * @category     WebHemi2
 * @package      WebHemi2_Component
 * @subpackage   WebHemi2_Component_Image
 * @author       Gixx @ www.gixx-web.com
 * @copyright    Copyright (c) 2014, Gixx-web (http://www.gixx-web.com)
 * @license      http://webhemi.gixx-web.com/license/new-bsd   New BSD License
 */
class Png extends AbstractImage
{
    /** No compression: best quality but big file size. */
    const COMPRESSION_BEST = 0;
    /** Best compression: low quality and small file size. */
    const COMPRESSION_WORST = 9;
    /** By dafault there is no filter used for the output */
    const IMAGE_FILTER = PNG_NO_FILTER;

    /**
     * @var int $quality   Compression index of the output if supported.
     */
    protected $quality;
    /**
     * @var int $imageFilter   PNG filter(s) used on output.
     */
    protected $imageFilter;
    /**
     * @var array $availableFilters   The list of the available filters for PNG format.
     */
    protected $availableFilters = array(
        PNG_NO_FILTER,
        PNG_FILTER_NONE,
        PNG_FILTER_SUB,
        PNG_FILTER_UP,
        PNG_FILTER_AVG,
        PNG_FILTER_PAETH,
        PNG_ALL_FILTERS
    );

    /**
     * Set image compression.
     *
     * @param int $quality
     *
     * @throws Exception
     *
     * @return Png
     */
    public function setQuality($quality = self::COMPRESSION_BEST)
    {
        if (!is_integer($quality)) {
            throw new Exception('Parameter is not valid.');
        }

        if ($quality < self::COMPRESSION_BEST || $quality > self::COMPRESSION_WORST) {
            throw new Exception('Parameter is out of range.');
        }

        $this->quality = $quality;

        return $this;
    }

    /**
     * Set a PNG filter for the output.
     *
     * @param int $imageFilter
     *
     * @throws Exception
     *
     * @return Png
     */
    public function setFilter($imageFilter = self::IMAGE_FILTER)
    {
        if (!in_array($imageFilter, $this->availableFilters)) {
            throw new Exception('Parameter is not valid.');
        }

        $this->imageFilter |= $imageFilter;

        return $this;
    }

    /**
     * Unset a PNG filter.
     *
     * @param int $imageFilter
     *
     * @throws Exception
     *
     * @return Png
     */
    public function unsetFilter($imageFilter = self::IMAGE_FILTER)
    {
        if (!in_array($imageFilter, $this->availableFilters)) {
            throw new Exception('Parameter is not valid.');
        }

        $this->availableFilters ^= $imageFilter;

        return $this;
    }

    /**
     * Reset filters to default.
     *
     * @return Png
     */
    public function resetFilters()
    {
        $this->imageFilter = self::IMAGE_FILTER;

        return $this;
    }

    /**
     * Read image file and create resource.
     *
     * @param string $fileName
     *
     * @throws Exception
     *
     * @return Png
     */
	public function readImage($fileName)
    {
        if (!is_null($fileName) && file_exists($fileName) && is_readable($fileName)) {
            if ($resource = @imagecreatefrompng($fileName)) {
                imagesavealpha($resource, true);
                imagealphablending($resource, false);
                $this->imageResource[0] = $resource;
            } else {
                throw new Exception('Resource is not a PNG image');
            }
        } else {
            throw new Exception('File not found or not readable');
        }
        return $this;
    }

    /**
     * Write image resource into Png file.
     *
     * @param string $fileName   Name of the Png file. If null the file will be written on stdout.
     *
     * @throws Exception
     */
    public function writeImage($fileName = null)
    {
        if ($resource = $this->getResource(0)) {
            if (is_null($fileName)) {
                header('Content-type: image/png');
            }

            imagepng($resource, $fileName, $this->quality, $this->imageFilter);
            imagedestroy($resource);

            if (!is_null($fileName)) {
                @chmod($fileName, $this->chmod);
            }
        } else {
            throw new Exception('No image resource available!');
        }
    }
}
