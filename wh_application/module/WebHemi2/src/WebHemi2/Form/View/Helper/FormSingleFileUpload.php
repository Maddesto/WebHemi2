...
class FormSingleFileUpload extends FormFile
{
    ...
    public function render(ElementInterface $element)
    {
        ...
        if (array_key_exists('data-mdl', $attributes)) {
            $output = <<<EOH
                <input class="mdl-textfield__input" type="text" data-placeholder="{$element->getLabel()}" name="{$element->getAttribute('id')}FileName" id="{$element->getAttribute('id')}FileName" readonly/>
                ...
                <script type="text/javascript">
                    document.getElementById("{$element->getAttribute('id')}").onchange = function () {
                        document.getElementById("{$element->getAttribute('id')}FileName").click();
                        document.getElementById("{$element->getAttribute('id')}FileName").value = this.files[0].name;
                    };
                </script>
EOH;
        } else {
